...
fun makeFunAux (n: int, xs: (plcType * string) list, e: expr): expr =
    case xs of [] => e
            | (h::tl) => Let((#2 h), Item(n, Var "$list"), (makeFunAux (n+1, tl, e)));

(* Catch the first elements in a tuple list. *)
fun catchFirstElement(l: (plcType * string) list) : plcType list =
  case l of [] => []
          | (h::tl) => ((#1 h)::(catchFirstElement tl));

(* Create the list of arguments of a function. *)
fun makeType (args: (plcType * string) list): plcType =
    case args of [] => ListT []
              |  (h::tl) => ListT(catchFirstElement (h::tl));
